...
function renderWatched(items) {
  watchedList.innerHTML = items.length ? items.map((item) => `<article class="recent-watch-row"><div class="recent-watch-copy"><strong>${escapeHtml(item.film?.title || "Untitled film")}${item.rewatch ? ' <em>rewatch</em>' : ""}</strong><span>${escapeHtml(item.date)} · ${venueLabel(item.venue) || "watch"}${item.rating !== null && item.rating !== undefined ? ` · ★ ${Number(item.rating).toFixed(1)}` : ""}</span>${item.note ? `<p class="recent-watch-note"><small>quick note</small>${escapeHtml(item.note)}</p>` : ""}</div>${item.film?.poster ? `<img class="recent-watch-poster" src="${escapeHtml(item.film.poster)}" alt="" />` : ""}</article>`).join("") : '<p class="empty-state">No unwritten screenings yet.</p>';
}
